...
El diagnóstico temprano de enfermedades como el cáncer de mama es una tarea prioritaria en la medicina moderna y el análisis de datos puede permitir detectar patrones que colaboren con el personal de salud en el diagnóstico preciso de algunas enfermedades. 

El método de redes neuronales utiliza aprendizaje supervisado y permite detectar patrones  que pueden  pasar desapercibidos cuando se utilizan otros métodos de aprendizaje automático.

Los Autoencoders son un tipo de redes neuronales, que constituyen un buen candidato para esta tarea porque están diseñados para aprender representaciones compactas con las características más relevantes de los datos, permitiendo distinguir entre anomalías y datos normales.

\subsubsection{Hipótesis}

Este trabajo se basa en la hipótesis de que al entrenar el Autoencoder unicamente con datos normales, los registros anómalos (malignos) tendrán un mayor error de reconstrucción $\lVert \hat{t} - t \rVert$ en comparación con los normales (benignos). Para decidir cuál es mínimo error de reconstrucción para el cual se decide que los datos provienen de una anomalía es necesario utilizar un umbral. Este umbral de decisión se calcula con el índice de Youden.

\subsubsection{Estado del arte}

Se desarrollaron múltiples variantes de los Autoencoders, evaluadas por los autores Neloy y Turgeon~\parencite{Acomprehensivestudyofautoencodersforanomalydetection}. Los cuales comparan la capacidad de reconstrucción, generación de muestras, visualización del espacio latente y precisión en clasificación de datos anómalos en 11 arquitecturas. Demostrando una mayor capacidad para detección de anomalías los modelos Algunas de las arquitecturas utilizadas son Denosing
...
Por otra parte, los autores Bouman y Heskes~\parencite{DBLP:journals/corr/abs-2501-13864}, cuestionan la capacidad de los autocodficadores
para detectar anomalías mediante el error de reconstrucción. Los autores afirman
que las anomalías lejanas a los datos normales pueden ser reconstruidas perfectamente, denotando una
falta de fiabilidad en los modelos.

Otras propuestas como la de Nawaz et al.~\parencite{Ensembleofautoencodersforanomaly} incluyen a los \emph{Ensemble of Autoencoders} como alternativa más robusta al uso de un solo modelo.
...
Para evaluar el desempeño del autocodificador en la tarea de detección de anomalías, se analizan métricas derivadas del error de reconstrucción $\lVert \hat{t} - t \rVert$. Este valor se utiliza como criterio de decisión para distinguir entre casos normales y anómalos mediante un umbral $\epsilon$, calculado con el índice de Youden. El registro se considera anomalía si lo supera.
...
A partir de esta regla se construye la matriz de confusión, y se calculan las métricas de exactitud, sensibilidad y especificidad, entre otras. Asimismo, se emplean la curva ROC y el área bajo dicha curva (AUC) para caracterizar el rendimiento del modelo sin depender de un valor de umbral fijo.
